...
export const Editor: React.FC<EditorProps> = ({ user, design, initialType = 'OTHER', onBack }) => {
    ...
    const toggleEditMode = () => {
        if (isEditable) {
            ...
            // Clear image selection when exiting edit mode
    ...
    // Handle click on image in edit mode
    const handleImageClick = (e: React.MouseEvent) => {
        ...
        if (target.tagName === 'IMG') {
            ...
            setImageToolbarPos({ x: rect.left + rect.width / 2, y: rect.top - 50 });
        } else {
            setSelectedImage(null);
            setImageToolbarPos(null);
        }
                ...
                <div className="flex-1 bg-[#0c0c0e] relative overflow-hidden flex flex-col">
                    ...
                    <div className="flex-1 overflow-hidden relative z-10">
                        {generatedCode || isPreparing ? (
                            <div className="w-full h-full overflow-y-auto bg-[#0c0c0e] p-4 md:p-8 lg:p-12 flex justify-center custom-scrollbar">
                                ...
                                {/* Image Edit Toolbar - Floating */}
                                {isEditable && selectedImage && imageToolbarPos && (
                                    <div
                                        className="fixed z-[100] bg-zinc-900 border border-white/20 rounded-xl shadow-2xl px-2 py-1.5 flex items-center gap-1 animate-in fade-in zoom-in-95 duration-200"
                                        style={{ left: imageToolbarPos.x, top: imageToolbarPos.y, transform: 'translateX(-50%)' }}
                                    >
                                        <button onClick={() => imageInputRef.current?.click()} className="p-2 hover:bg-white/10 rounded-lg text-white transition-colors" title="Replace Image">
                                            <i className="ph ph-swap text-lg"></i>
                                        </button>
                                        <button onClick={handleDeleteImage} className="p-2 hover:bg-red-500/20 rounded-lg text-red-400 transition-colors" title="Delete Image">
                                ...
                                <input ref={imageInputRef} type="file" accept="image/*" onChange={handleReplaceImage} className="hidden" />
                                ...
                                <div
                                    ...
                                    className={`flex flex-col gap-6 md:gap-12 pb-32 transition-opacity duration-500 w-full max-w-full ${isEditable ? 'cursor-text ring-2 ring-green-500/20 rounded-lg p-2' : 'cursor-default'}`}
                                    dangerouslySetInnerHTML={{ __html: cleanHTML(generatedCode) }}
                                />
                            </div>
                        ) : (
